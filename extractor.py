...
class Worker(object):
    ...
    def run(self):
        ...
        cad = cut_and_downmix.CutAndDownmix(center=self._center, input_sample_rate=self._sample_rate,
        ...
        while True:
            basename, time_stamp, signal_strength, freq, signal = self._work_queue.get()
            try:
                msg = None
                try:
...
if __name__ == "__main__":
    ...
    def process_one(basename, time_stamp, signal_strength, freq, signal):
        work_queue.put((basename, time_stamp, signal_strength, freq, signal))
        #time.sleep(1)
    ...
    def wrap_process(time_stamp, signal_strength, freq, signal):
        ...
        queue_len += 1
        in_count += 1
        process_one(basename, time_stamp, signal_strength, freq, signal)
